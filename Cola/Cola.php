...
class Cola {
    ...
    public static function loadClass($className, $classFile = '') {
        ...
        if ((! $classFile) && ('Cola' === substr ( $className, 0, 4 ))) {
            $classFile = dirname ( COLA_DIR ) . DIRECTORY_SEPARATOR . str_replace ( '_', DIRECTORY_SEPARATOR, $className ) . '.php';
        }
        ...
        /**
         * auto load data class
         */
        if ((! $classFile) && ('Data' === substr ( $className, - 4 ))) {
            $classFile = self::getConfig ( '_datasHome' ) . "/{$className}.php";
        }
        
        /**
         * auto load data class
         */
        if ((! $classFile) && ('Tool' === substr ( $className, - 4 ))) {
            $classFile = self::getConfig ( '_toolsHome' ) . "/{$className}.php";
        }
    ...
    public static function setClassPath($class, $path = '') {
        ...
        self::getInstance ()->config->merge ( array (
            '_class' => $class 
        ) );
