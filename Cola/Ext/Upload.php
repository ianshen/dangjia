...
class Cola_Ext_Upload {
    ...
    public function files() {
        ...
        foreach ( $_FILES as $field => $data ) {
            ...
            if (is_string ( $data ['name'] )) {
                $files [] = $data + array (
                    'field' => $field, 
                    'ext' => $this->getExt ( $data ['name'], true ) 
                );
                continue;
            }
    ...
    public function move($file, $name = null) {
        ...
        $this->error = array (
            'code' => - 26, 
            'msg' => "{$dir}: Failed to move uploaded file" 
        );
        return false;
    }
    ...
    public function check($file) {
        if (UPLOAD_ERR_OK != $file ['error']) {
            ...
            $this->error = array (
                'code' => 0 - $code, 
                'msg' => "{$file['name']}: {$this->config['error'][$code]}" 
            );
            return false;
        }
        ...
        $checks = array (
            'checkType' => array (
                'code' => - 21, 
                'msg' => "{$file['name']}: File type not allowed" 
            ), 
            'checkExt' => array (
                'code' => - 22, 
                'msg' => "{$file['name']}: File ext not allowed" 
            ), 
            'checkFileSize' => array (
                'code' => - 23, 
                'msg' => "{$file['name']}: File size not allowed" 
            ), 
            'checkImageSize' => array (
                'code' => - 24, 
                'msg' => "{$file['name']}: Image size not allowed" 
            ) 
        );
